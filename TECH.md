...
v3 是一个多阶段分配算法，解决 v2 的以下问题：
- 总量小的作业（如「作文 5 篇」横跨 60 天）被拆得太散
- 没有优先级概念，无法控制「先写物理再写化学」
- 没有间隔分配，无法实现「隔天做一次」
...
权重公式（与 v2 相同）：
- rhythm=-2（极左/前紧后松）：w(t) = 1 - 0.7t
- rhythm=-1（偏左）：w(t) = 1 - 0.35t
- rhythm=0（均匀）：w(t) = 1.0
- rhythm=1（偏右）：w(t) = 0.65 + 0.35t
- rhythm=2（极右/前松后紧）：w(t) = 0.3 + 0.7t

对每一天 i（0..N-1），t = i/(N-1)，权重 w_i = w(t)。

#### 3.2.3 多阶段分配流程

```
输入：homework[] (含 priority/interval_days/window_start/window_end/locked)
     rhythm, maxDailyMinutes, dates[] (可用日期列表)

阶段 0：预处理
  - 过滤 locked 的作业（不改动）
  - 按 priority ASC 排序（priority 小 = 优先分配）
  - 计算 dates[] 的天数 N

阶段 1：间隔过滤
  对每项作业 hw：
    - 如果 hw.interval_days = 0 → 窗口内所有天都可用
    - 如果 hw.interval_days = k (k≥1) → 每隔 k 天取一天
      间隔从窗口起始日（或计划起始日）开始算
      例：k=1（隔天），起始日 7/10，窗口 [7/10, 7/20]
      可用：7/10, 7/12, 7/14, 7/16, 7/18, 7/20
    - 过滤后得到 each_hw.available_days[]

阶段 2：自动间隔推荐
  对每项作业 hw：
    - 窗口内有效天数 = M
    - 如果 interval_days 未手动设置（初始值 0）：
        推荐间隔 = max(0, floor(M / totalAmount) - 1)
        例：作文 5 篇 / 60 天 → floor(60/5)-1 = 11 → 每 12 天一篇
        例：练习册 60 页 / 60 天 → floor(60/60)-1 = 0 → 每天 1 页
    - 如果 interval_days > 0：使用用户设定值
    - 按 interval_days 过滤可用天：从窗口起始日算，每隔 interval_days 天取一天
    - 得到每项作业的 effective_days[]

阶段 3：节奏权重
  生成每日权重 w[0..N-1]（同 v2）

阶段 4：按优先级顺序分配（核心改进）
  daily_load[d] = 0 for d in [0..N-1]  // 每天已分配的分钟数
  
  按 priority 从高到低（从小到大）遍历每项作业 hw：
    - 取 hw.effective_days（已按间隔过滤）
    - 对每个可用天 d，计算该天的剩余容量：
        remaining[d] = maxDailyMinutes - daily_load[d]
    - 按 v2 权重比例分配 totalAmount 单位到 effective_days，
      但受剩余容量约束：如果当天容量不够放 1 单位，
      跳过该天，寻找下一个容量够的天
    - 更新 daily_load[d]

  效果：
    - 优先级高的作业先占满容量，优先级低的被挤到后期
    - 间隔过滤使总量小的任务不每天出现
    - 容量不足时自动跳到下一个可用天

阶段 4b：预计完成日期
  - 扫描 daily_load[]，找到最后一天 load > 0 的日期 → 预计完成日期
  - 写入 allocated_range 字段（每项作业的最早/最晚分配日期）

阶段 5：整数取整
  与 v2 相同：按权重比例取整，余数按权重排序分配

阶段 6：溢出处理
  如果某天超过 maxDailyMinutes：
    - 优先把这天中优先级最低的任务挪到相邻天
    - 如果所有相邻天都满了 → 记录警告

阶段 7：科目轮转排序
  每天内的任务按 subject 排序展示
  本阶段不改变分配，只影响展示顺序
```
...
| 改进 | v2 | v3 |
|------|----|----|
| 优先级 | 无 | priority 字段拖拽排序，高优先先占容量 |
| 自动间隔 | 无，始终每天分配 | `floor(窗口天数/总量)-1` 自动推荐 |
| 手动间隔 | 无 | interval_days，0=每天，1=隔天… |
| 溢出策略 | 按大小挪 | 按优先级低先挪 |
| 预计完成 | 无 | 最后有作业的日期，可滑块调整 |
| 作文问题 | 5 篇散到 60 天 | 自动间隔 11 天一篇 + 低优先级自然排后期 |

#### 3.2.5 预计完成日期调整逻辑

```
用户滑块值 targetCompletion（计划起始日 .. 计划结束日）
  → 如果 targetCompletion < 当前预计完成日期（调早）：
      按比例增加每天的分配量/减少间隔
      → 实质是乘一个缩放因子
  → 如果 targetCompletion > 当前预计完成日期（调晚）：
      按比例减少每天的分配量
      → 超出计划结束日时无效（clamp）

实现方式：
  - 作为 preview/generate API 的新可选参数 targetEndDate
  - 传了此参数时，算法在阶段 3 后尝试缩放分配量
  - 缩放策略：全局乘因子 (原始预计天数 / 目标天数)
  - 如果调太早导致容量不够 → 记录 warning
```
